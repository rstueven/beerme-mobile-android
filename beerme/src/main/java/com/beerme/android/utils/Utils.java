...
*/
    ...
    public static URL buildUrl(String file, String[] parameters)
        ...
        StringBuilder buffer = new StringBuilder(Utils.BEERME_URL + file);
        buffer.append("?appVersion=").append(getAppVersion());
        buffer.append("&platformVersion=").append(getPlatformVersion());
    ...
    public static String metersToUnits(Context context, float meters) {
        ...
        return String.format(Locale.getDefault(), "%.1f %s", meters * Float.parseFloat(conversion[index]), units[index]);
    }
    ...
    public static LatLngBounds expandBounds(Context context, LatLngBounds orig, int d) {
        ...
        try {
            ...
            newBounds = orig;
            ErrLog.log(context, "Utils.expandBounds", e, R.string.Cant_create_map);
        }
    ...
    public static LatLng offsetPoint(LatLng orig, double dist, int bearing) {
        ...
        double lat = Math.asin(Math.sin(latRadians) * Math.cos(distRadians) + Math.cos(latRadians) * Math.sin(distRadians) * Math.cos(bearingRadians));
        double lng;
        if (Math.cos(lat) == 0) {
            lng = lngRadians;
        } else {
            lng = lngRadians + Math.atan2(Math.sin(bearingRadians) * Math.sin(distRadians) * Math.cos(latRadians), Math.cos(distRadians) - Math.sin(latRadians) * Math.sin(lat));
        }
    ...
    public static String breweryStatusString(Context context, int status) {
        return context.getResources().getStringArray(R.array.status_value)[BreweryStatusFilterPreference.getIndex(status)];
    }

    public static boolean isNavigationAvailable(Activity activity) {
        Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("google.navigation:q=41.25,-96"));
