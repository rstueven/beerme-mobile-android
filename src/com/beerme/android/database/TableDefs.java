...
public abstract class TableDefs extends Fragment {
	...
	protected static String createIndex(String table, String name,
			String[] columns) {
		StringBuffer buffer = new StringBuffer("CREATE INDEX " + table + "_"
				+ name + " ON " + table + " (");
	...
	protected static void setLastUpdate(Context context) {
		...
		editor.putString(Prefs.KEY_DB_LAST_UPDATE, DbOpenHelper.sqlDateFormat
				.format(Calendar.getInstance().getTime()));
		saver.savePreferences(editor, false);
	}

	protected static String getLastUpdate(Context context) {
		return Prefs.getSettings(context).getString(Prefs.KEY_DB_LAST_UPDATE,
				Utils.DISTANT_PAST);
	}

	protected static void resetLastUpdate(Context context) {
		SharedPreferences.Editor editor = Prefs.getSettingsEditor(context);
		SharedPreferenceSaver saver = Prefs.getSettingsSaver(context);
		editor.putString(Prefs.KEY_DB_LAST_UPDATE, Utils.DISTANT_PAST);
		saver.savePreferences(editor, false);
	}
	...
	protected static void startDownloadProgress(Context context, int title,
			int id) {
