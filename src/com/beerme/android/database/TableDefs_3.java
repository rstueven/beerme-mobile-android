...
public class TableDefs_3 extends TableDefs {
	...
	protected void upgrade(SQLiteDatabase db) {
		...
		try {
			...
			db.execSQL(String.format("INSERT INTO %s (%s) SELECT %s from temp",
					TABLE_BREWERYNOTES, cols, cols));
			...
			c = db.rawQuery(
					"SELECT pagenumber, beerid, package, score, sampled, place, appearance, aroma, mouthfeel, notes, breweryid FROM temp",
					null);
			...
			if (c != null) {
				String insertSql = "INSERT INTO "
						...
						+ " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
				SQLiteStatement insertStatement = db
						.compileStatement(insertSql);
			...
			ErrLog.log(mContext, "onUpgrade(case 2)", e,
